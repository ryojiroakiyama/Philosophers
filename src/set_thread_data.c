...
static void	set_thread_data_philo(t_manage_data *mdata, t_thread_data *philo, int philo_index)
{
	...
	philo->mutex[TO_RIGHT_FORK] = mdata->mutexies + mdata->mutexinfo[FORKS][INDEX] + philo_index;
	philo->mutex[TO_LEFT_FORK] = mdata->mutexies + mdata->mutexinfo[FORKS][INDEX] + ((philo_index + 1) % mdata->philo_num);
	philo->mutex[TO_LAST_EAT] = mdata->mutexies + mdata->mutexinfo[LASTEATS][INDEX] + philo_index;
	philo->mutex[TO_PUT] = mdata->mutexies + mdata->mutexinfo[PUTS][INDEX];
	philo->mutex[TO_LIFE_FLAG] = mdata->mutexies + mdata->mutexinfo[LIFES][INDEX];
